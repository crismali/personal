...
- `build-csp.ts` — CSP hashing module used by `build.ts`. Hashes (SHA-256) the inline `<script>` and `<style>` blocks in the built HTML and writes them into `dist/_headers`, replacing the `__INLINE_SCRIPT_HASH__` and `__INLINE_STYLE_HASHES__` placeholders so the CSP `script-src`/`style-src` directives avoid `'unsafe-inline'`.
