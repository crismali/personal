...
- `build-csp.ts` — CSP hashing module used by `build.ts`. Hashes (SHA-256) every inline `<script>` and `<style>` block in the built HTML and writes them into `dist/_headers`, replacing the `__INLINE_SCRIPT_HASH__` and `__INLINE_STYLE_HASHES__` placeholders so the CSP `script-src`/`style-src` directives avoid `'unsafe-inline'`. Matches lazily (`[\s\S]*?`) so content containing a literal `<` (e.g. `a<b`) hashes correctly instead of truncating. Only matches bare `<script>`/`<style>` tags with no attributes — tags with attributes (`type="module"`, `type="application/ld+json"`, etc.) are intentionally skipped. Throws if a placeholder or matching tag is missing, but does **not** catch inline `style="..."` attributes — those aren't hashed and will be silently blocked by `style-src` if ever added.
- `__tests__/` — Bun test files for the root build scripts (e.g. `build-csp.ts`).
